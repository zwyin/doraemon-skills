---
name: doraemon-project-walkthrough
version: "1.6.1"
...
```
/doraemon-project-walkthrough [path] [--depth brief|medium|deep|all] [--audience general|dev] [--lang zh|zh-pure|en|bilingual] [--no-confirm] [--version]
```
...
**Examples:**
```
/doraemon-project-walkthrough                                              # Brief + general + zh (CWD)
/doraemon-project-walkthrough /path/to/project                             # Brief + general + zh
/doraemon-project-walkthrough /path/to/project --depth medium              # Medium + general + zh
/doraemon-project-walkthrough /path/to/project --depth deep --audience dev # Deep + dev + zh
/doraemon-project-walkthrough --depth deep                                 # Deep + general + zh (CWD)
/doraemon-project-walkthrough --audience dev                               # Brief + dev + zh (CWD)
/doraemon-project-walkthrough --depth all --audience dev                   # All depths + dev + zh (CWD)
/doraemon-project-walkthrough ./deep --depth brief                         # Path "./deep", brief + general + zh
/doraemon-project-walkthrough /path/to/project --lang en                   # Brief + general + en
/doraemon-project-walkthrough --depth deep --lang en                       # Deep + general + en (CWD)
/doraemon-project-walkthrough --depth deep --no-confirm                    # Deep + skip confirmation
/doraemon-project-walkthrough /path/to/project --lang zh-pure              # Brief + general + zh-pure
/doraemon-project-walkthrough /path/to/project --lang bilingual            # Brief + general + bilingual
/doraemon-project-walkthrough --version                                    # Print version and exit
```
...
The output directory name MUST use the skill's name prefix: `<prefix>_project_study_<project-name>/`, where `<prefix>` comes from the `name` field in frontmatter (e.g., `ruyi` from `doraemon-project-walkthrough` → `ruyi_project_study_my-codebase/`).
...
If the script outputs an upgrade reminder, inform the user: "A new version is available. You can continue with the current version, or run `claude plugin update doraemon-skills@doraemon-skills` to upgrade."
