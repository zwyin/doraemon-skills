...
- **修改方案**: 替换为 `直接在 doraemon-skills 开发 → convert.sh → git commit → brand-sync-tool 分发`
- **风险**: 无 — 纯文档修改

### #6 目录结构 skill 名错误
- **位置**: 文档目录结构（原第 24-27 行）
- **问题**: 写的是 `github-safe-publish/`、`project-walkthrough/`，实际是 `doraemon-github-safe-publish/`、`doraemon-project-walkthrough/`
- **修改方案**: 更新为实际目录名
- **风险**: 无 — 纯文档修改
...
- **修改方案**: 改为"直接在 doraemon-skills 仓库中维护"
- **风险**: 无 — 纯文档措辞修改
...
- **问题**: 文档说平台文件输出到 gitignored 的 dist/，实际 doraemon-project-walkthrough 的平台文件（cursor/.windsurf/.opencode）已 git 跟踪并提交在 skill 目录内；doraemon-github-safe-publish 的平台文件在 dist/ 下未被跟踪。两个 skill 策略不一致
...
### #13 新增：已归档仓库章节
- **修改方案**: 说明 github-safe-publish 和 project-walkthrough-skill 已 archived，代码统一维护在 doraemon-skills
- **风险**: 无
